...
function fmtExpiresIn(iso: string, t: any): string {
  ...
  if (days >= 1) return t('settings.members.expires_in_days', { count: days });
  const hours = Math.max(1, Math.floor(ms / (60 * 60 * 1000)));
  return t('settings.members.expires_in_hours', { count: hours });
}
...
export function MembersTab() {
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      if (!mres.ok) {
        ...
        toast.error(payload.error || t('settings.members.toasts.load_members_failed'));
        return;
      }
      ...
      if (ires) {
        if (!ires.ok) {
          ...
          toast.error(payload.error || t('settings.members.toasts.load_invites_failed'));
          return;
        }
      ...
      toast.error(t('settings.members.toasts.server_error'));
    } finally {
      setLoading(false);
    }
  }, [canManageMembers]);
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('settings.members.toasts.update_role_failed'));
        return;
      }
      toast.success(t('settings.members.toasts.updated_role', { 
        name: member.full_name || t('settings.members.toasts.member_fallback'),
        role: t(`settings.roles.${nextRole}`)
      }));
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('settings.members.toasts.remove_failed'));
        return;
      }
      toast.success(t('settings.members.toasts.removed', { 
        name: removingMember.full_name || t('settings.members.toasts.member_fallback')
      }));
      setMembers((prev) =>
  ...
  async function handleRevoke(invite: Invitation) {
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('settings.members.toasts.revoke_failed'));
        return;
      }
      toast.success(t('settings.members.toasts.revoked'));
        ...
        action={
          <RequireRole min="admin">
            <Button onClick={() => setInviteOpen(true)}>
              ...
              {t('settings.members.invite_member')}
            </Button>
          </RequireRole>
        }
      />
          ...
          return (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5">
                ...
                {counts.online} {t('settings.members.status_online')}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <PresenceDot status="away" />
                {counts.away} {t('settings.members.status_away')}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <PresenceDot status="offline" />
                {counts.offline} {t('settings.members.status_offline')}
              </span>
              <span className="text-muted-foreground/70">
                · {members.length} {t(`settings.members.member_count_${members.length === 1 ? 'one' : 'other'}`)}
              </span>
            </div>
          );
        })()}
              ...
              return (
                ...
                >
                          ...
                          <Avatar className="size-9 shrink-0">
                            {member.avatar_url ? (
                              <AvatarImage
                                ...
                                alt={member.full_name || t('settings.members.unnamed')}
                              />
                            ) : null}
                  ...
                  <div className="hidden sm:block text-right text-xs text-muted-foreground">
                    {t('settings.members.joined')} {fmtDate(member.joined_at, i18n.language)}
                  </div>
        ...
        <div>
          <div className="mb-2 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-foreground">
              {t('settings.members.pending_invitations')}
            </h3>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              {t('settings.members.invite_warning')}
            </p>
          ) : null}
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  {t('settings.members.no_invites')}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t('settings.members.no_invites_help_1')} <span className="text-muted-foreground">{t('settings.members.no_invites_help_2')}</span>{' '}
                  {t('settings.members.no_invites_help_3')}
                </p>
              </CardContent>
            </Card>
          ) : (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-foreground">
                            {inv.label || t('settings.members.untitled_invite')}
                          </span>
                          <span
                          ...
                          >
                            ...
                            {t(`settings.roles.${inv.role}`)}
                          </span>
                        </div>
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          {t('settings.members.created')} {fmtDate(inv.created_at, i18n.language)} · {fmtExpiresIn(inv.expires_at, t)}
                        </p>
                      </div>
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-popover-foreground">
              ...
              {t('settings.members.remove_title')}
            </DialogTitle>
            <DialogDescription className="text-muted-foreground">
              {t('settings.members.remove_desc_1')}{' '}
              <span className="font-medium text-muted-foreground">
                {removingMember?.full_name || t('settings.members.unnamed')}
              </span>{' '}
              {t('settings.members.remove_desc_2')}
            </DialogDescription>
          </DialogHeader>
