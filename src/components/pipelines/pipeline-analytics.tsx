    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          label={t("pipelines.analytics.total_deals")}
          value={String(stats.totalCount)}
          tooltip={t("pipelines.analytics.tooltip_total_deals")}
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-primary" />}
          label={t("pipelines.analytics.pipeline_value")}
          value={formatCurrency(stats.totalValue, defaultCurrency)}
          tooltip={t("pipelines.analytics.tooltip_pipeline_value")}
        />
        <Metric
          icon={<Target className="h-4 w-4 text-blue-400" />}
          label={t("pipelines.analytics.avg_deal_size")}
          value={formatCurrency(stats.avgValue, defaultCurrency)}
          tooltip={t("pipelines.analytics.tooltip_avg_deal_size")}
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-purple-400" />}
          label={t("pipelines.analytics.weighted_value")}
          value={formatCurrency(stats.weightedValue, defaultCurrency)}
          tooltip={t("pipelines.analytics.tooltip_weighted_value")}
        />
        <Metric
          icon={<Trophy className="h-4 w-4 text-primary" />}
          label={t("pipelines.analytics.won_this_month")}
          value={String(stats.wonThisMonth)}
          tooltip={t("pipelines.analytics.tooltip_won_this_month")}
        />
        <Metric
          icon={<XCircle className="h-4 w-4 text-red-400" />}
          label={t("pipelines.analytics.lost_this_month")}
          value={String(stats.lostThisMonth)}
          tooltip={t("pipelines.analytics.tooltip_lost_this_month")}
        />
      </div>
    </TooltipProvider>
  );
        ...
        <Tooltip>
          <TooltipTrigger
            render={
              <button
                type="button"
                aria-label={t("pipelines.analytics.how_label_calculated", { label })}
