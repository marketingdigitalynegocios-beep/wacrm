  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        <div>
          <h2 className="text-sm font-semibold text-foreground">{t('dashboard.conversations_chart.title')}</h2>
          <p className="mt-0.5 text-xs text-muted-foreground">{t('dashboard.conversations_chart.subtitle')}</p>
        </div>
        <div className="flex items-center gap-1 rounded-lg bg-muted/60 p-1">
          {[7, 30, 90].map((r) => (
            ...
            >
              {t('dashboard.conversations_chart.days', { count: r })}
            </button>
          ))}
        </div>
      </header>
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={MessageSquare}
            title={t('dashboard.conversations_chart.empty_title')}
            hint={t('dashboard.conversations_chart.empty_hint')}
          />
        ) : (
          <LineSvg data={data} maxY={maxY} ticks={niceTicks} t={t} locale={i18n.language} />
        )}
      </div>

      <footer className="flex items-center gap-4 border-t border-border px-5 py-3 text-xs text-muted-foreground">
        <LegendDot color="#3b82f6" label={t('dashboard.conversations_chart.incoming')} />
        <LegendDot color="#7c3aed" label={t('dashboard.conversations_chart.outgoing')} />
      </footer>
    </section>
  )
}
        ...
        >
          ...
          <div className="mt-1 flex flex-col gap-0.5">
            <span className="flex items-center gap-1.5 text-blue-300">
              ...
              {t('dashboard.conversations_chart.incoming_count', { count: hovered.incoming })}
            </span>
            <span className="flex items-center gap-1.5 text-primary">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-primary" />
              {t('dashboard.conversations_chart.outgoing_count', { count: hovered.outgoing })}
            </span>
          </div>
        </div>
      )}
