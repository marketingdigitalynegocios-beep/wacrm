  ...
  return (
    <section className="rounded-xl border border-border bg-card">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        <h2 className="text-sm font-semibold text-foreground">{t('dashboard.activity_feed.title')}</h2>
        <Link
          href="/inbox"
          className="text-xs font-medium text-primary hover:text-primary/80"
        >
          {t('dashboard.activity_feed.view_all')}
        </Link>
      </header>
      ...
      {loading || !items ? (
        ...
        <div className="p-5">
          <EmptyState
            icon={Inbox}
            title={t('dashboard.activity_feed.empty_title')}
            hint={t('dashboard.activity_feed.empty_hint')}
          />
        </div>
      ) : (
        <>
              ...
              const row = (
                <div className="flex items-center gap-3 px-5 py-2.5">
                  ...
                  <span className="flex-shrink-0 text-xs text-muted-foreground tabular-nums">
                    {relativeTime(it.at, t, i18n.language)}
                  </span>
                </div>
              )
              return (
          ...
          <footer className="flex items-center justify-between border-t border-border px-5 py-3 text-xs">
            <span className="text-muted-foreground tabular-nums">
              {t('dashboard.activity_feed.showing', { visible: visible.length, total: totalLoaded, plus: totalLoaded === 50 ? '+' : '' })}
            </span>
            <div className="flex items-center gap-1">
              <span className="mr-1 text-muted-foreground">{t('dashboard.activity_feed.show')}</span>
...
function relativeTime(iso: string, t: any, locale: string): string {
  const then = new Date(iso).getTime()
  if (Number.isNaN(then)) return ''
  const diffSec = Math.round((Date.now() - then) / 1000)
  if (diffSec < 60) return t('dashboard.activity_feed.relative_time.seconds', { count: Math.max(1, diffSec) })
  if (diffSec < 3600) return t('dashboard.activity_feed.relative_time.minutes', { count: Math.floor(diffSec / 60) })
  if (diffSec < 86400) return t('dashboard.activity_feed.relative_time.hours', { count: Math.floor(diffSec / 3600) })
  if (diffSec < 2_592_000) return t('dashboard.activity_feed.relative_time.days', { count: Math.floor(diffSec / 86400) })
