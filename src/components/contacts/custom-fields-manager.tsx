  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t('contacts.custom_fields.title')}</DialogTitle>
...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (isDuplicate(name)) {
      toast.error(t('contacts.custom_fields.duplicate', { name }));
      return;
    }
    ...
    if (error) {
      toast.error(t('contacts.custom_fields.create_error'));
      return;
    }
    toast.success(t('contacts.custom_fields.created', { name }));
    setNewName('');
    await fetchFields();
  }
  ...
  async function handleRename(
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.custom_fields.rename_error'));
      return false;
    }
    await fetchFields();
    return true;
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        t('contacts.custom_fields.confirm_delete', { name: field.field_name })
      )
    ) {
      return;
    }
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.custom_fields.delete_error'));
      return;
    }
    toast.success(t('contacts.custom_fields.deleted', { name: field.field_name }));
    await fetchFields();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          placeholder={t('contacts.custom_fields.new_placeholder')}
      ...
      <div className="max-h-72 overflow-y-auto rounded-md border border-border">
        {loading ? (
          <div className="flex items-center justify-center gap-2 py-8 text-sm text-muted-foreground">
            <Loader2 className="size-4 animate-spin" />
            {t('contacts.custom_fields.loading')}
          </div>
        ) : fields.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">
            {t('contacts.custom_fields.empty')}
          </p>
        ) : (
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={t('contacts.custom_fields.rename_aria', { name: field.field_name })}
      ...
      <Button
        ...
        title={t('contacts.custom_fields.delete_title')}
