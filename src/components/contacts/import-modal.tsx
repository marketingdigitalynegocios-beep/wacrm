      ...
      {tagNames.map((name) => {
        ...
        return (
          <span
            ...
            title={isKnown ? name : `${name} ${t('contacts.import.will_create_tag')}`}
          >
            <span
...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(t('contacts.import.invalid_csv'));
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
      setTagColorByKey(new Map());
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (imported > 0) {
        toast.success(t('contacts.import.imported', { count: imported }));
        onImported();
      }
      if (tagsAssigned > 0) {
        toast.success(t('contacts.import.tags_assigned', { count: tagsAssigned }));
      }
      ...
      if (skipped > 0) {
        toast.info(t('contacts.import.skipped', { count: skipped }));
      }
      if (failed > 0) {
        toast.error(t('contacts.import.failed', { count: failed }));
      }
    } catch (err: unknown) {
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              <Trans i18nKey="contacts.import.description">
                Upload a CSV file with a required <code className="rounded bg-muted px-1 py-0.5 text-[11px] text-muted-foreground">phone</code> column. Optional:{' '}
                <code className="rounded bg-muted px-1 py-0.5 text-[11px] text-muted-foreground">name</code>
                ,{' '}
                <code className="rounded bg-muted px-1 py-0.5 text-[11px] text-muted-foreground">email</code>
                ,{' '}
                <code className="rounded bg-muted px-1 py-0.5 text-[11px] text-muted-foreground">company</code>
                ,{' '}
            ...
            {file ? (
              <>
                ...
                <span className="rounded-full bg-muted px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                  {t('contacts.import.rows_ready', { count: parsedRows.length })}
                </span>
              </>
            ) : (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  {t('contacts.import.click_to_choose')}
                </p>
                <p className="text-[11px] text-muted-foreground">
                  {t('contacts.import.csv_limit')}
                </p>
              </>
            )}
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4">
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground uppercase">
                  {t('contacts.import.preview', { count: preview.length })}
                </p>
                <div className="flex flex-wrap items-center gap-1.5">
                  {tagStats.rowsWithTags > 0 && (
                    <span className="inline-flex items-center gap-1 rounded-md bg-muted/90 px-2 py-0.5 text-[11px] text-muted-foreground">
                      ...
                      {t('contacts.import.tags_assigned', { count: tagStats.unique })} ·{' '}
                      {t('contacts.import.imported', { count: tagStats.rowsWithTags })}
                    </span>
                  )}
                </div>
              </div>
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  {t('contacts.import.rows_not_shown', { count: parsedRows.length - PREVIEW_LIMIT })}
                </p>
              )}
            </div>
          )}
          ...
          {result && (
            <div className="rounded-xl border border-border bg-background/50 p-4">
              <p className="text-sm font-medium text-popover-foreground">{t('contacts.import.import_complete')}</p>
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                {result.tagsAssigned > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    <CheckCircle className="size-4 shrink-0" />
                    {t('contacts.import.tags_assigned', { count: result.tagsAssigned })}
                  </div>
                )}
                {result.skipped > 0 && (
        ...
        <DialogFooter className="mt-0 shrink-0 gap-2 border-t border-border/80 bg-background/50 px-6 py-4 sm:justify-end">
          ...
          {!result && (
            ...
            >
              ...
              {t('contacts.import.import_button', { count: parsedRows.length })}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
